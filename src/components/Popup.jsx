...
const Popup = (props) => {
    ...
    return (
        <ModalBox
            open={props.show}
            sx={{ border: 0, boxShadow: 0, width: 450, borderRadius: 3, p: 1, }}>
            <Box sx={{
                p: 2,
                ...
                <Box>
                    <Box sx={{ "& img": { height: '70%', width: '70%' } }}>
                        <Box sx={{ width: '60px', height: '60px', mx: 'auto', borderRadius: '50%', p: 1, background: `linear-gradient(#fff 50%, ${bgColor} )`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Box sx={{ mt: 3.5, textAlign: 'center' }}>
                        <Box sx={{ mb: 1.5 }}>
                            <Typography variant='span' sx={{ fontSize: { xs: "14px", md: '18px' }, fontWeight: 500, color: "#121212", letterSpacing: .3 }}>{props.title}</Typography>
                        </Box>
                        {props.children}
                    </Box>
                </Box>
                <Box sx={{ mt: 5 }}>
                    <Stack gap={2} alignItems="center" sx={{ flexDirection: 'row', justifyContent: 'center' }}>
                        <Button size='medium' sx={{ px: 4, py: 1, ...props.secondaryBtnStyles, borderColor: '#d5d5d5', color: '#222', lineHeight: 'normal', letterSpacing: .3 }} color={'inherit'} variant='outlined' onClick={(e) => { props.onSecondaryClick(e, setIsSecondaryLoading) }}>
                        ...
                        <Button color={props.variant === "error" ? "error" : "primary"} sx={{ px: 4, py: 1.2, ...props.primaryBtnStyles, lineHeight: 'normal', letterSpacing: .3 }} variant="contained" onClick={(e) => props.onPrimaryClick(e, setIsPrimaryLoading)}>
                            {props.primaryBtnTxt}
